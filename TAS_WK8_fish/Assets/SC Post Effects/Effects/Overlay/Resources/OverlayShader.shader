Shader "Hidden/SC Post Effects/Overlay"
{
	...
	float4 Frag(VaryingsDefault i) : SV_Target
	{
		...
		if (_BlendMode == 0) color = lerp(screenColor.rgb, overlay.rgb, overlay.a * _Params.x);
		if (_BlendMode == 1) color = lerp(screenColor.rgb, BlendAdditive(overlay.rgb, screenColor.rgb), overlay.a * _Params.x);
		if (_BlendMode == 2) color = lerp(screenColor.rgb, overlay.rgb * screenColor.rgb, overlay.a * _Params.x);
		if (_BlendMode == 3) color = lerp(screenColor.rgb, BlendScreen(overlay.rgb, screenColor.rgb), overlay.a * _Params.x);
