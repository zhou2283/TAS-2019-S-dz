...
public class BezierSpline : MonoBehaviour {
	...
	private void Start()
	{
		timeToDistanceArray = new float[splineDiv];
		GetLength();
	}
	...
	//b curve distance based
	public float GetLength()
	{
		length = 0;
		for (int i = 0; i < splineDiv - 1; i++)
		{
	...
	{
		return (float)index / (float)splineDiv;
	}
	...
	public float GetTimeWithDistance (float d) {
		for (int i = 0; i < timeToDistanceArray.Length - 1; i++)
		{
			if (d > timeToDistanceArray[i] && d <= timeToDistanceArray[i + 1])
			{
				...
				{
					return IndexToTime(i+1);
				}

				return (IndexToTime(i) + IndexToTime(i + 1)) / 2f;
			}
